...
1. To generate the synthetic datasets, you will need to run the script `src/simulation/generate_data_R0_run.jl`. This script will generate a synthetic dataset of viral load trajectories and save it in the `data` folder. 
2. Next, run the script `src/simulation/process_sim_data_robustness.jl` to process the data. 
3. Now we can run the inference on the synthetic dataset. Run the script `src/inference_log/within_host_inference_robustness_run.jl`. This will run the within-host model on the synthetic datasets and save the results in a csv in the `results` folder.
...
2. We run `src/inference_log/process_nba_data.jl` script to process the NBA dataset. This will save the processed data in the `data/nba` folder.
3. Now we can run the inference on the NBA dataset. Run the script `src/inference_log/within_host_inference_nba_run.jl`. This will run the within-host model on the NBA dataset and save the results in a csv in the `results/nba` folder.
...
- `src/analysis/results_posteriors.jl` — this script will generate the posteriors for the results of the within-host model for the a specific synthetic dataset.
- `src/analysis/results_simulations.jl` — this script will generate posterior predictive VL trajectories for the within-host model for a synthetic dataset.
...
- `src/analysis/robustness_analysis.jl` — this script produces the coverage plot and computes the various estimates in the paper.
