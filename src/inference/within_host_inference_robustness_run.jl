...
# (data, ids) = get_cleaned_data(data_dir("sims/sim_data_clean.csv"))
...
function intialise_params()
    ...
    ϕ₀ = SharedParams(
        μ_R₀, log(σ_R₀), μ_k, log(σ_k), μ_δ, log(σ_δ), μ_πv, log(σ_πv), μ_c, log(σ_c), 0.5
    )
...
# laplace_approx_full_likelihood($θ₀[1], $data[1], $ϕ₀, $M)
# end
